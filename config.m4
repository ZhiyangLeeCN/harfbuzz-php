...
if test "$PHP_HARFBUZZ" != "no"; then
    ...
    LIB_HARFBUZZ_CXX_FLAGS="-DHAVE_ICU -DHAVE_ICU_BUILTIN -DHAVE_INTEL_ATOMIC_PRIMITIVES -DHAVE_ROUNDF -DHB_NO_MMAP -DHB_NO_RESOURCE_FORK \
      -DHB_NO_SUBSET_LAYOUT -DHB_NO_SUBSET_CFF -DHB_NO_FALLBACK_SHAPE -DHB_NO_UCD -DHB_NO_WIN1256 -DHB_EXPERIMENTAL_API"
    ...
    PHP_ADD_SOURCES_X($HARFBUZZ_EXIT_DIR, $LIB_ICU_SOURCES, $PHP_HARFBUZZ_CXX_FLAGS, shared_objects_harfbuzz, yes)
    ...
    PHP_ADD_SOURCES_X($HARFBUZZ_EXIT_DIR, $LIB_ICU_I18N_SOURCES, $PHP_HARFBUZZ_CXX_FLAGS, shared_objects_harfbuzz, yes)
